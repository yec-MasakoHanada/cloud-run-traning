...
```plain
asia-northeast1-docker.pkg.dev/env95-cloudruntraining/repo-hanada/ws-flask-app:latest
```
...
```plain
docker build -t asia-northeast1-docker.pkg.dev/env95-cloudruntraining/repo-hanada/ws-flask-app:latest
```
...
```plain
docker push asia-northeast1-docker.pkg.dev/env95-cloudruntraining/repo-hanada/ws-flask-app:latest
```

## Cloud Runにデプロイ

```bash
$ gcloud run deploy cloud-run-hanada \
    --image asia-northeast1-docker.pkg.dev/env95-cloudruntraining/repo-hanada/ws-flask-app:latest \
...
```bash
$ gcloud run services proxy clound-run-hanada --region asia-northeast1 --port 8082
```
